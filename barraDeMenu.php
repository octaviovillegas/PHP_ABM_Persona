<div id="header">
		
			<a class="btn btn-primary animated bounceInLeft" href="index.php"><span class="glyphicon glyphicon-list-alt">&nbsp;</span>Menu principal</a>
			<a class="btn btn-primary animated bounceInLeft" href="grilla.php"> <span class="glyphicon glyphicon-th">&nbsp;</span>   Grilla</a>
			<a class="btn btn-primary animated bounceInLeft" href="alta.php"><span class="glyphicon glyphicon-plus-sign">&nbsp;</span>Alta</a>
		

			<span id="tituloBarra"  class="animated bounceInRight">ABM -  Versión 1.0.2 con archivo de texto y Con foto</span>

	</div>